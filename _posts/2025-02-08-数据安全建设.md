...
分类分级工具：通过实际业务环境进行自研，若业务较为简单也可以使用现有数据分类分级工具（如 Symantec Data Loss Prevention、Microsoft Purview）自动识别和分类数据。
```
#数据识别简单示例
import re
...
## 3. 数据使用阶段
数据访问控制：
身份认证：使用多因素认证（MFA）确保用户身份的真实性。例如，使用 Google Authenticator 或 Microsoft Authenticator及自研。
